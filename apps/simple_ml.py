...
def parse_mnist(image_filename, label_filename):
    ...
    """
    ### BEGIN YOUR SOLUTION
    with gzip.open(image_filename, "rb") as image_file, gzip.open(
        label_filename, "rb"
    ) as label_file:
...
def softmax_loss(Z: ndl.Tensor, y_one_hot: ndl.Tensor):
    ...
    return (ndl.ops.log((ndl.ops.exp(Z)).sum(axes=1)) - Z_y).sum() / Z.shape[0]

    ### END YOUR SOLUTION


def nn_epoch(
    X: np.ndarray, y: np.ndarray, W1: ndl.Tensor, W2: ndl.Tensor, lr=0.1, batch=100
):
    ...
    """
    N, input = X.shape
    hidden, output = W2.shape

    for i in range(0, N, batch):
        X_batch, y_batch = ndl.Tensor(X[i : i + batch]), y[i : i + batch]

        Z1 = ndl.ops.relu(X_batch @ W1)

        I_y = np.zeros((y_batch.size, output))
        I_y[np.arange(y_batch.size), y_batch] = 1
        I_y = ndl.Tensor(I_y)

        Z2 = Z1 @ W2

        L = softmax_loss(Z2, I_y)
