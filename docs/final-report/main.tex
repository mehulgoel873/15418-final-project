...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Nsight Compute traces for $N = 32768, G = 8, p = 0.95$, $N = 1024, G = 8, p = 0.95$ runs. Per-kernel stalls: \texttt{long\_scoreboard} flags warps waiting on global-memory loads, \texttt{mio\_throttle} flags warps waiting on shared-memory loads, \texttt{barrier} flags warps waiting on a synchronization call}. 
