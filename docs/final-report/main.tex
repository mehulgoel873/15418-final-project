...
\paragraph{Active-coordinate gather.}
% Read this over @Saransh, I made assumptions I'm not sure if it's corect
The version we ended up using inverts the order of operations: first gather the positions that need to be computed, then compute only those. The output is partitioned into $32 \times 32$ SDDMM tiles, which sit on top of the smaller $T \times T$ BCSR blocks of the mask (with $T \le 32$). One warp owns each SDDMM tile and processes it in three stages.

\begin{enumerate}
    \item \textbf{Dynamic tile scheduling.} Warps pull the next $32 \times 32$ tile via an atomic increment of a global counter, rather than from a static grid. Warps that finish quickly grab another tile, and warps that land on a fully-sparse tile move on after almost no work.
    \item \textbf{Active-coordinate list.} The warp walks the BCSR row-pointers of $M$ to find the dense $T \times T$ blocks that intersect its $32 \times 32$ tile, and appends the $(r, c)$ coordinates of every entry inside those blocks to a per-warp list in shared memory. If the list is empty after this pass, the warp is done with the tile.
    \item \textbf{Dense load, sparse compute, sparse write-back.} The warp iterates the $N$ dimension in $32$-wide chunks. Each chunk loads the full $32 \times 32$ stripes of $A$ and $B$ into shared memory with regular coalesced reads, then computes a dot product \emph{only} at the coordinates in the active list. After all $N$-chunks finish, the accumulators are written back into the BCSR storage of $C$ at exactly those positions.
\end{enumerate}
