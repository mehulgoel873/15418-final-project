...
#define TILE 4
    ...
    CudaDims CudaTwoDim(size_t x, size_t y)
    {
       ...
       */
      CudaDims dim;
      size_t x_blocks = (x + V - 1) / V;
      size_t y_blocks = (y + V - 1) / V;
      dim.block = dim3(V, V, 1);
...
#define MAX_VEC_SIZE 8
                                  ...
                                  CudaVec strides, size_t offset)
    {
      ...
      uint64_t cur_gid = gid;
      uint64_t a_idx = offset;
      for (int i = shape.size - 1; i >= 0; i--)
      {
        a_idx += (cur_gid % shape.data[i]) * strides.data[i];
        cur_gid /= shape.data[i];
      }
      if (gid < size)
      {
        out[gid] = a[a_idx];
    ...
    {
      /**
       * The CUDA kernel for the ewise setitem operation.  This should effectively map a single entry in the
       * compact input a, to the corresponding item (at location gid) in the not-compact array out.
       *
       * Args:
       *   a: CUDA pointer to a array
       *   out: CUDA point to out array (not-compact)
       *   size: size of out array
       *   shape: vector of shapes of a and out arrays (of type CudaVec, for past passing to CUDA kernel)
       *   strides: vector of strides of out array
       *   offset: offset of out array
      ...
      for (int i = shape.size - 1; i >= 0; i--)
      {
        idx += (cur_gid % shape.data[i]) * strides.data[i];
        cur_gid /= shape.data[i];
      }
      if (gid < size)
      {
    ...
    void ScalarMul(const CudaArray &a, scalar_t val, CudaArray *out)
    {
      ...
      // Launch the ScalarMulKernel that will Mul the scalar 'val' to each element of array 'a',
      // and store the result in array 'out'.
    ...
    void ScalarDiv(const CudaArray &a, scalar_t val, CudaArray *out)
    {
      ...
      // Launch the ScalarDivKernel that will Div the scalar 'val' to each element of array 'a',
      // and store the result in array 'out'.
    ...
    void ScalarPower(const CudaArray &a, scalar_t val, CudaArray *out)
    {
      ...
      // Launch the ScalarPowerKernel that will Div the scalar 'val' to each element of array 'a',
      // and store the result in array 'out'.
    ...
    void ScalarMaximum(const CudaArray &a, scalar_t val, CudaArray *out)
    {
      ...
      // Launch the ScalarMaximumKernel that will Max the scalar 'val' to each element of array 'a',
      // and store the result in array 'out'.
    ...
    void ScalarEq(const CudaArray &a, scalar_t val, CudaArray *out)
    {
      ...
      // Launch the ScalarEqKernel that will Eq the scalar 'val' to each element of array 'a',
      // and store the result in array 'out'.
    ...
    void ScalarGe(const CudaArray &a, scalar_t val, CudaArray *out)
    {
      ...
      // Launch the ScalarGeKernel that will Ge the scalar 'val' to each element of array 'a',
      // and store the result in array 'out'.
    ...
    __global__ void MatmulKernel(const scalar_t *a, const scalar_t *b, scalar_t *out, uint32_t M, uint32_t N,
                                 uint32_t P)
    {
      uint32_t i_base = blockIdx.x * blockDim.x + threadIdx.x;
      uint32_t j_base = blockIdx.y * blockDim.y + threadIdx.y;

      for (uint32_t k = 0; k < N; k++)
      {
        for (uint32_t i = i_base * V; i < min(M, (i_base + 1) * V); i++)
        {
          for (uint32_t j = j_base * V; j < min(P, (j_base + 1) * V); j++)
          {
            // out[0] = 1.0;
            out[i * P + j] += a[i * N + k] * b[k * P + j];
          }
    ...
    __global__ void ReduceSumKernel(scalar_t *a, scalar_t *out, size_t size, size_t reduce_size)
    {
      ...
      if (gid < size)
      {
        ...
        for (size_t i = 0; i < reduce_size; i++)
        {
          out[gid] = out[gid] + a[gid * reduce_size + i];
        }
      }
    }
