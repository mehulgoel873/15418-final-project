...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        probs = ops.mul_scalar(probs, (q_dim ** -0.5))
        if self.causal:
            mask = Tensor(
                self.create_causal_mask(
                    queries_len, keys_values_len, device=self.device
                ),
                device=self.device,
                dtype=self.dtype,
            )
            # print(probs.shape, mask.shape)
            probs += ops.broadcast_to(mask, probs.shape)
            # print(probs)
        probs = self.softmax(probs)
        probs = self.dropout(probs)
        v_transpose_compact = ops.transpose(v).realize_cached_data().compact()
        v_transpose_compact = Tensor(v_transpose_compact, device=v.device, dtype=v.dtype, requires_grad=False)
        result = self.matmul(probs, v_transpose_compact)
        ### END YOUR SOLUTION
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        q_prime : Tensor = self.prenorm_q(q)
        q_prime : Tensor = self.prenorm_q(q)
        q_prime : Tensor = self.prenorm_q(q)

        q_prime: Tensor = self.q_projection(q_prime)
        k_prime: Tensor = self.k_projection(self.prenorm_k(k))
        v_prime: Tensor = self.v_projection(self.prenorm_v(v))

        q_prime = q_prime.reshape(
            (batch_size, queries_len, self.num_head, self.dim_head)
        ).transpose(axes=(1, 2))
        k_prime = k_prime.reshape(
            (batch_size, keys_values_len, self.num_head, self.dim_head)
        ).transpose(axes=(1, 2))
        v_prime = v_prime.reshape(
            (batch_size, keys_values_len, self.num_head, self.dim_head)
        ).transpose(axes=(1, 2))

        q_compact = Tensor(q_prime.realize_cached_data().compact(), device=q.device, dtype=q.dtype, requires_grad=False)
        k_compact = Tensor(k_prime.realize_cached_data().compact(), device=k.device, dtype=k.dtype, requires_grad=False)
        v_compact = Tensor(v_prime.realize_cached_data().compact(), device=v.device, dtype=v.dtype, requires_grad=False)

        X_heads, _ = self.attn(q_compact, k_compact, v_compact)

        X_heads = ops.transpose(X_heads, axes=(1, 2))
        X_heads_compact = Tensor(X_heads.realize_cached_data().compact(), device=X_heads.device, dtype=X_heads.dtype, requires_grad=False)
        X_heads_compact = ops.reshape(X_heads_compact, (batch_size, queries_len, self.num_head * self.dim_head))
        X_heads_compact = ops.reshape(X_heads_compact, (batch_size * queries_len, self.num_head * self.dim_head))
        result = self.out_projection(X_heads_compact)
...
class Transformer(Module):
    ...
    def forward(self, x, h=None):
        ...
        position_np = np.arange(seq_len).reshape((1, seq_len))
        position_enc = Tensor(
            array=position_np,
            device=x.device,
            dtype=x.dtype
        ...
        position_enc = position_enc.broadcast_to(x.shape[:-1])
        x += self.pos_embedding(position_enc)
