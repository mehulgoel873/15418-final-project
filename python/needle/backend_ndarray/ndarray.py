...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if -1 in new_shape:
            ...
            assert math.prod(self.shape) % (-1 * math.prod(new_shape)) == 0
            new_shape[new_shape.index(-1)] = math.prod(self.shape) // (
                -1 * math.prod(new_shape)
            )
    ...
    def permute(self, new_axes: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        assert sorted(new_axes) == list(range(self.ndim))
        new_strides = tuple([self.strides[i] for i in new_axes])
        new_shapes = tuple([self.shape[i] for i in new_axes])
        out = self.make(
            shape=new_shapes,
            strides=new_strides,  # FIX
            device=self.device,
            handle=self._handle,
            offset=self._offset,
        )
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        new_strides = [0 for _ in new_shape]
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        if not isinstance(idxs, tuple):
            idxs = tuple(
                idxs if i == 0 else slice(None, None, None) for i in range(self.ndim)
            )
        slices = tuple(
            [
        ...
        for i, s in enumerate(slices):
            ...
            new_stride.append(self.strides[i] * step)
    ...
    def reduce_view_out(
        ...
        """Return a view of the array and an output array for reduction ops."""

        if axis is None:
            # Reduce over all elements
            view = self.compact().reshape((1, prod(self.shape)))
            out_shape = (1,) if not keepdims else tuple(1 for _ in self.shape)
            out = NDArray.make(out_shape, device=self.device)
            return view, out

        # Normalize and handle multiple axes
        if isinstance(axis, int):
            axis = (axis,)
        axis = tuple(sorted(a if a >= 0 else a + self.ndim for a in axis))

        # Permute so that reduction axes are at the end
        non_reduce_axes = [a for a in range(self.ndim) if a not in axis]
        perm = non_reduce_axes + list(axis)
        view = self.permute(perm).compact()

        # Collapse dims: [outer, reduce]
        outer_dim = prod([self.shape[i] for i in non_reduce_axes])
        reduce_dim = prod([self.shape[i] for i in axis])
        view = view.reshape((outer_dim, reduce_dim))
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert isinstance(axes, tuple), "Axes for flip are required to be tuples"

        new_strides = [
            -self.strides[i] if i in axes else self.strides[i] for i in range(self.ndim)
        ]
        new_offset = self._offset
        for axis in axes:
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        assert (
            len(axes) == self.ndim
        ), "Pad must be supplied same number of axes as ndim"

        new_shape = [self.shape[i] + axes[i][0] + axes[i][1] for i in range(self.ndim)]
        slices = tuple(
            slice(axes[i][0], axes[i][0] + self.shape[i]) for i in range(self.ndim)
        )
        out = self.device.full(shape=new_shape, fill_value=0.0, dtype=self.dtype)
        out[slices] = self

        return out
        ### END YOUR SOLUTION

    def im2_col(self, K: int, stride: int) -> "NDArray":
        assert self.ndim == 4
        N, H, W, C = self.shape
        Ns, Hs, Ws, Cs = self.strides
        return self.make(
            shape=(N, (H - K + 1) // stride, (W - K + 1) // stride, K, K, C),
...
def squeeze(array: NDArray) -> NDArray:
...
def im2_col(a: NDArray, K: int, stride: int) -> NDArray:
    return a.im2_col(K, stride)
